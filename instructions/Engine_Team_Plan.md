...
for each row:
    for each column:
        if board[row][column] is an integer:
            append board[row][column] to opennings
...
def cpu_vs_cpu(x, o):
    ...
    forever:
        show the board
        Take CPU x's turn
        Check to see if the game is over; break the loop if X has one or drawn

        show the board
        Take CPU O's turn
        Check to see if the game is over; break the loop if O has one or drawn
...
This structure ensures that the logo is printed only at the start of the program, or after a game has ended.  We don't want to spam the user with that big block of text everytime they make a typo!
...
We intentionally avoided new-fangled features of Python, as well as things that we can reasonably expect to be deprecating in upcoming versions.  This Tic-Tac-Toe game should keep on ticking!
